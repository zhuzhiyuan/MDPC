...
var initCopyAndPaste=function(tempWindow,tempDocument){


    (function($){
        $(tempWindow).on('dragover',function(e){
            var $target = $(e.target);
        ...
        if(window.isMac){
            $(tempDocument).on("keydown",function(e){
                //cmd+a
                if(e.metaKey&& e.keyCode==65){
                    tempDocument.execCommand("selectAll");
                    e.preventDefault();
                }
                //cmd+x
        ...
        $(tempDocument).on('contextmenu', function (e) {
            console.log('running contextmenu');
            var $target = $(e.target);
            var selectionType = tempWindow.getSelection().type.toUpperCase();
            if ($target.is(':text') || $target.is(':input') || selectionType === 'RANGE') {   // TODO url/email/... 未加入判断哦
                var clipData = gui.Clipboard.get().get();
                var menu = new Menu(tempDocument);
        ...
        console.log('end contextmenu');
    })(jQuery);
}
initCopyAndPaste(window,document);//页面事件初始化
var appendFrameContextmenuListener = function(frame){//iframe事件初始化
    var subWindow = frame.contentWindow;
    var subDocument = subWindow.document;
    if(window.isMac){
        subDocument.onkeydown=function(){
            var e=event||arguments[0];
            console.log(e);
            if(e.metaKey&& e.keyCode==65){
                subDocument.execCommand("selectAll");
                e.preventDefault();
            }
            //cmd+x
            if(e.metaKey && e.keyCode==88){
                subDocument.execCommand("cut");
                e.preventDefault();
            }
            //cmd+c
            if(e.metaKey && e.keyCode==67){
                subDocument.execCommand("copy");
                e.preventDefault();
            }
            //cmd+v
            if(e.metaKey && e.keyCode==86){
                subDocument.execCommand("paste");
                e.preventDefault();
            }
        }
    }
    subDocument.oncontextmenu = function () {
       var e=event||arguments[0];
        var $target = $(e.target);
        var selectionType = subWindow.getSelection().type.toUpperCase();
        if ($target.is(':text') || $target.is(':input') || selectionType === 'RANGE') {   // TODO url/email/... 未加入判断哦
            var clipData = gui.Clipboard.get().get();
            var menu = new Menu(subDocument);
            menu.canPaste(clipData.length > 0);
            menu.canCopy(selectionType === 'RANGE');
            var x= jQuery("#"+frame.id).offset().left+ e.clientX;
            var y= jQuery("#"+frame.id).offset().top+ e.clientY;
            menu.popup(x, y);
            e.preventDefault();
        }
    }
...
win.on("document-end",function(frame){
    if(frame){
        appendFrameContextmenuListener(frame);
    }
});
/**
 * tray
